...
const menuItems = [
  { id: 'village', label: 'Moje sołectwo', icon: MapPin, value: 'Tymbark' },
...
function ProfilePage() {
  return (
    <div className="px-5 pt-4 space-y-5">
      ...
      <div className="card-base p-5">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-bold text-graphite">Anna Nowak</h2>
            <p className="text-xs text-graphite-light">Sołectwo Tymbark • od 2024</p>
            <div className="flex items-center gap-1 mt-1">
              <div className="px-2 py-0.5 bg-mint-light/20 rounded-full">
                <span className="text-[10px] font-semibold text-forest">Aktywny mieszkaniec</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Logout */}
      <button className="w-full flex items-center justify-center gap-2 py-3 text-sm text-red-400 font-medium hover:text-red-500 transition-colors">
        <LogOut size={16} />
        Wyloguj się
      </button>
    </div>
