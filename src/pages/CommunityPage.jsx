          ...
          {group.messages.length === 0 ? (
            ...
            group.messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.from === currentUser?.name ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[75%] px-3 py-2 rounded-2xl text-[11px] ${msg.from === currentUser?.name
      ...
      <div className="flex bg-white rounded-xl border border-card-border p-1 shadow-card">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`flex-1 py-2 rounded-lg text-[11px] font-semibold transition-all duration-200 ${tab === t.id ? 'bg-forest text-white shadow-sm' : 'text-graphite-light hover:text-graphite'
              }`}
          >
            {t.label}
          </button>
          ...
          <div className="flex gap-1.5 overflow-x-auto scrollbar-hide -mx-4 px-4">
            {categories.map(cat => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-[10px] font-semibold whitespace-nowrap transition-all border ${categoryFilter === cat
